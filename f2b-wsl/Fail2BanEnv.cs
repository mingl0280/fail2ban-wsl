    ...
    public class Fail2BanEnv : IDisposable
    {
        ...
        private void FileContentChangeHandle(string filePath)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                if ((string.IsNullOrEmpty(lineContent) || lineContent.Length < 7))
                { continue; }
                else
                {
                    CachedDummyFileContent.Add(lineContent);
                    foreach (IPlugin plugin in _parentService.Plugins)
                    {
                        plugin.RaiseOnBadIPDetected(this, new PluginEventArgs() { EventSource = _f2bDirectoryString, EventTextContent = lineContent, EventType = "NewBadIPEvent" });
                    }
                }
            }
            sr.Close();
